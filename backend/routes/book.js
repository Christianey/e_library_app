...
async function fileUpload(req, res, next) {
  if(!req.user) return res.status(403).json("You are not permitted to view this resource")

  const user = await User.findById(req.user.id)

  if(!user) return res.status(401).json("You are not authorized to view this resource")

  user
  try {
    if(!req.file) res.status(400).json({message: "File missing. Make sure it is being sent."});
...
async function getUserBooks(req, res, next) {
  try {
    ...
    if (username !== usernameParams) return res.status(403).json("You are not permitted to view this resource")
